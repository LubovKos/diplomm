...
public class MarchingCubes {
    ...
    public static List<Triangle> extractSurface(Grid grid) {
    ...
    private static void processCube(Grid grid, int x, int y, int z, List<Triangle> triangles) {
        double[] cubeValues = new double[8];
        cubeValues[0] = grid.getSDF(x, y, z);
        cubeValues[1] = grid.getSDF(x + 1, y, z);
        cubeValues[2] = grid.getSDF(x + 1, y, z + 1);
        cubeValues[3] = grid.getSDF(x, y, z + 1);
        cubeValues[4] = grid.getSDF(x, y + 1, z);
        cubeValues[5] = grid.getSDF(x + 1, y + 1, z);
        cubeValues[6] = grid.getSDF(x + 1, y + 1, z + 1);
        cubeValues[7] = grid.getSDF(x, y + 1, z + 1);

        int configIndex = 0;
        for (int i = 0; i < 8; i++)
            if (cubeValues[i] < 0) configIndex |= (1 << i);
        ...
        for (int edge = 0; edge < 12; edge++) {
            ...
            vertList[edge] = interpolateEdge(x, y, z, edge, cubeValues);
        }

        int[] triIndices = triTable[configIndex];
        for (int i = 0; i < triIndices.length; i += 3) {
            if (triIndices[i] == -1) break;
            triangles.add(new Triangle(
                    vertList[triIndices[i]],
                    vertList[triIndices[i + 1]],
                    vertList[triIndices[i + 2]]
            ));
        }
    }

    private static Point3D interpolateEdge(int x, int y, int z, int edge, double[] cubeValues) {
        // Определяем вершины ребра (пример для edge 0)
        int v1, v2;
        switch (edge) {
            case 0:  v1 = 0; v2 = 1; break;
            case 1:  v1 = 1; v2 = 2; break;
            case 2:  v2 = 3; v1 = 2; break;
        ...
        double val1 = cubeValues[v1];
        double val2 = cubeValues[v2];
        double t = (0 - val1) / (val2 - val1); // Изоуровень S=0

        // Координаты вершин ребра в гриде
        double x1 = x + ((v1 == 0 || v1 == 3 || v1 == 4 || v1 == 7) ? 0 : 1);
        double y1 = y + ((v1 == 0 || v1 == 1 || v1 == 4 || v1 == 5) ? 0 : 1);
        double z1 = z + ((v1 < 4) ? 0 : 1);

        double x2 = x + ((v2 == 0 || v2 == 3 || v2 == 4 || v2 == 7) ? 0 : 1);
        double y2 = y + ((v2 == 0 || v2 == 1 || v2 == 4 || v2 == 5) ? 0 : 1);
        double z2 = z + ((v2 < 4) ? 0 : 1);

        // Интерполяция
        double ix = x1 + t * (x2 - x1);
        double iy = y1 + t * (y2 - y1);
        double iz = z1 + t * (z2 - z1);
